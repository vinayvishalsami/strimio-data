...
for channel_id, (channel_name, channel_url) in CHANNELS.items():
    ...
    for show in series:
        ...
        # -------- Phase 1: Discover NEW episodes only --------
        while should_continue:
            ...
            for a in links:
                ...
                if inferred_id and inferred_id in existing_ids:
                    if found_new:
                        confirmed_seen += 1
                else:
                    found_new = True
        ...
        # -------- Phase 2: Fetch NEW episode pages --------
        new_episodes = []
        ...
        for ep_url in new_urls:
            ...
            eid = f"{show['id']}_{m.group(3)}_{MONTHS[m.group(2)]}_{int(m.group(1)):02d}"
            ...
            new_episodes.append({
                "id": eid,
                "name": f"{show['name']} {m.group(1)}-{MONTHS[m.group(2)]}-{m.group(3)}",
                "links": links,
            })
        ...
        # -------- Merge and save --------
        merged = new_episodes + existing_eps
        seen = set()
        final = []
        for e in merged:
            if e["id"] not in seen:
